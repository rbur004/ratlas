...
*   INCX & INCY default to 1
  i.e. 
    x.axpy(y, a = 1.0, b = 1.0, n = x.ncols, incx = 1, incy = 1)
...
static VALUE rb_blas_xaxpy_mod(int argc, VALUE *argv, VALUE self)
{
  Matrix *dx, *dy;
  int incx;
  int incy;
  ...
  case Complex_t: //c
    ...
    {
      db_c[0] = NUM2DBL(rb_funcall( rb_intern("Complex"),  rb_intern("real"),  1, db_value) );
      db_c[1] = NUM2DBL(rb_funcall(rb_intern("Complex"),  rb_intern("image"),  1, db_value ) );
