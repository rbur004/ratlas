...
static VALUE rb_lapack_xgesv_mod(VALUE self, VALUE result, VALUE ipiv)
{
  Matrix *m, *r, *i;
  int error;
  int info;
  char *args[] = { "Number of equations in 'A'",
...
static VALUE rb_lapack_xgesv( VALUE self, VALUE result, VALUE ipiv )
{
  Matrix *dx, *dy;
  VALUE self_copy, result_copy;
    
  Data_Get_Struct(self, Matrix, dx);
  self_copy = rblas_new_instance(dx->class_id, dx->data, dx->nrows, dx->ncols, dx->data_type);
  Data_Get_Struct(result, Matrix, dy);
  result_copy = rblas_new_instance(dy->class_id, dy->data, dy->nrows, dy->ncols, dy->data_type);
