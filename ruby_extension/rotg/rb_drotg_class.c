...
static VALUE rblas_drotg_initialize(int argc, VALUE *argv, VALUE obj)
{
  ...
  if(a == Qnil) drotg->r = 0; else drotg->r = NUM2DBL(a);
  if(b == Qnil) drotg->z = 0; else drotg->z = NUM2DBL(b);
  if(c == Qnil) drotg->c = 0; else drotg->c = NUM2DBL(c);
  if(s == Qnil) drotg->z = 0; else drotg->s = NUM2DBL(s);
  if(a != Qnil && b != Qnil  && argc == 2) //if not nil then assume have to calculte values for c & s
    cblas_srotg(&drotg->r, &drotg->z, &drotg->c, &drotg->s);
