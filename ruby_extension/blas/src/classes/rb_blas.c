#define RB_BLAS_C
//Blas class. Parent of SingleBlas, DoubleBlas, ComplexBlas and DoubleComplexBlas
#include "rb_blas.h"
  ...
  else
  {
    ...
    {   
      ...
      { 
        ...
        row_dataPtr = RARRAY_PTR(argv[i]); //we know the bounds, so work with the col data values.
        if(ncols == 0)
        {
          //ncols = RARRAY(argv[i])->len; 
          ncols = (int) RARRAY_LEN(argv[i]); 
      ...
      {   //Then we have a Vector
        ...
        rb_blas_value_to_member(m, i-2, 0, argv[i]);
      }
    }
  }
...
static VALUE rblas_get_by_index(int argc, VALUE *argv, VALUE obj)
{
  ...
  return rb_blas_member_to_value(m, the_row, the_col);
}
...
static VALUE rblas_set_by_index(int argc, VALUE *argv, VALUE obj) 
{
  ...
  rb_blas_value_to_member(m, the_row, the_col, value);
  return value;
}
...
static VALUE rblas_each_by_row(VALUE obj)
{
  ...
  for(r = 0; r < m->nrows; r++)
    ...
    { //yield the double at [r,c], and give the row number, and column number.
      rb_yield(  rb_blas_member_to_value(m, r, c) );
    } 
  return Qnil; 
}
...
static VALUE rblas_each_row(VALUE obj) 
{
  ...
  if(m->ncols >= 1)
  {
    ...
    for(r = 0; r < m->nrows; r++)
    {
      for(c = 0; c < m->ncols; c++)
      {
        rb_blas_get_member((char *)row_data + (c * m->data_size), m, r, c);
      }
...
static VALUE rblas_each_by_row_with_index(VALUE obj)
{
  ...
  for(r = 0; r < m->nrows; r++)
    for(c = 0; c < m->ncols; c++)
    { //yield the double at [r,c], and give the row number, and column number.
      rb_yield( rb_ary_new3( 3, rb_blas_member_to_value(m, r, c), INT2FIX(r), INT2FIX(c) ) );
    } 
  return Qnil; 
}
...
static VALUE rblas_each_col(VALUE obj)
{
  ...
  if(m->ncols >= 1)
  {
    ...
    for(c = 0; c < m->ncols; c++)
    {
      for(r = 0; r < m->nrows; r++)
      {
        rb_blas_get_member((char *)column_data + (r * m->data_size), m, r, c);
      }
...
static VALUE rblas_to_a(VALUE obj)
{
  ...
  {
    for(r = 0; r < m->nrows; r++)
      rb_ary_store(matrix_array, r, rb_blas_member_to_value(m, r, 0) );
  }
  else
  {
    for(r = 0; r < m->nrows; r++)
    {
      ...
      for(c = 0; c < m->ncols; c++)
      { 
        rb_ary_store(row_array, c, rb_blas_member_to_value(m, r, c) ); //add in each column element of this row
      } 
    }
  }
...
static VALUE rblas_to_s(VALUE obj)
{
  ...
  if(m->ncols == 1) //a vector, so just store it in the matrix_array
  {
    ...
    for(r = 0; r < m->nrows; r++)
    {
      rb_blas_member_to_s(buffer, m, r, 0);
  ...
  else
  {
    ...
    for(r = 0; r < m->nrows; r++)
    {
      ...
      for(c = 0; c < m->ncols; c++)
      { 
        rb_blas_member_to_s(buffer, m, r, c);
        rb_str_cat(s, buffer, strlen(buffer) );
        if(c != m->ncols-1)
          rb_str_cat(s, ", ", 2 );
      } 
...
VALUE Init_blas(VALUE myModule) 
{
  ...
  Init_IntegerBlas(myModule, myClass);
  Init_SingleBlas(myModule, myClass);
  Init_DoubleBlas(myModule, myClass);
  Init_ComplexBlas(myModule, myClass);
  Init_DoubleComplexBlas(myModule, myClass);
